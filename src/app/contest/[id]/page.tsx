...
const ContestDetailsPage = async ({ params }: { params: { id: number } }) => {

    const contestById = await getContestById(params.id);
    console.log(contestById);

    const json = contestById[0]?.description;
    const output = json ? generateHTML(JSON.parse(json), defaultExtensions) : '';
